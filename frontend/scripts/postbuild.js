...
// 确保上级目录的 dist/frontend 存在
const rootDistPath = fsPath.join(currentDir, '..', 'dist');
createDirIfExists(rootDistPath);

const frontendDistPath = fsPath.join(rootDistPath, 'app');
recreateDir(frontendDistPath);
...
// 执行复制操作
copyRecursive(sourceDist, frontendDistPath);
console.log('Frontend build artifacts moved successfully to dist/frontend');